...
const HowItWorks = () => {
  const steps = [
    ...
    },
    {
      ...
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 text-humble-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="relative mt-16">
          ...
          <div className="absolute top-20 left-0 right-0 h-0.5 bg-humble-gray-700 hidden md:block">
            <div className="absolute top-0 left-0 h-full w-full bg-gradient-to-r from-humble-pink-500 via-humble-purple-500 to-humble-blue-500 transform origin-left"></div>
          </div>
            ...
            {steps.map((step, index) => (
              <div key={index} className="relative z-10">
                <div className="flex flex-col items-center">
                  <div className="bg-humble-charcoal rounded-full w-20 h-20 flex items-center justify-center mb-6 shadow-lg border border-humble-gray-700">
                    {step.icon}
                  </div>
                  
                  <div className="text-3xl font-bold text-humble-pink-500 mb-3">{step.number}</div>
                ...
                {index < steps.length - 1 && (
                  <div className="hidden md:block absolute top-20 right-0 transform translate-x-1/2 -translate-y-1/2">
                    <ArrowRight className="h-8 w-8 text-humble-purple-500" />
                  </div>
                )}
              </div>
            ))}
